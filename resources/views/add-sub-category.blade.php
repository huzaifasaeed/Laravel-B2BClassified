...
<h1 class="text-center title-1"> Add Sub Category </h1>
...
<form action="{{ url('/add-new-sub-categories' ) }}" method="POST" class="form-horizontal">
            {{ csrf_field() }}
...
@endforeach
            ...
            <!-- Task Name -->
            <div class="form-group">
                <label for="sub categorie" class="col-sm-3 control-label">Sub Categories</label>

                <div class="col-sm-8">
                    <input type="text" name="name" id="Sub-categories" class="form-control">
                </div>
            </div>
        ...
        @if($category->id==$subcategory->categorie_id)
        <tr>
        <td>
            {{$subcategory->id}}
        </td>
